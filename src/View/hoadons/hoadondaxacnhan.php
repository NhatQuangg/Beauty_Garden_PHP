...
<h5 class="card-title">Danh sách hóa đơn đã xác nhận</h5>
	<!-- Đã xác nhận -->
		...
		<thead>
			...
			<?php foreach ($hoadons as $hoadon): ?>
				<tr style="vertical-align: middle;">
					<a href="/hoadon/hoadondaxacnhanList/<?= $hoadon['mahoadon'] ?>">
					...
					<td style="text-align: center;">
                        <a href="/hoadon/delete/<?= $hoadon['mahoadon'] ?>">Xóa</a>
					</td>
					</tr>
            <?php endforeach; ?>
			</tbody>							
	...
	<script>
		function confirmDelete() {
			...
			var result = confirm("Bạn có chắc chắn muốn xóa toàn bộ không?");
				
			// Kiểm tra giá trị trả về từ hộp thoại xác nhận
			if (result) {
				// Chuyển hướng đến trang xóa
				window.location.href = "/hoadon/deleteall/";
			} else {
